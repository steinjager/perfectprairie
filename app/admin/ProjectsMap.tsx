...
import { parseMap, type Project, type Client } from "@/lib/admin-types";
import { addRoadLabels } from "@/lib/road-labels";
import { defaultCenter } from "@/lib/map";
import { imageryLayer } from "@/lib/imagery-layer";

export default function ProjectsMap({ projects, clients, onOpen }: { projects: Project[]; clients:Client[]; onOpen: (project: Project) => void }) {
  ...
  useEffect(()=>{
    ...
    void import("leaflet").then(L=>{
      ...
      const removeRoads=addRoadLabels(L,map);cleanup=()=>{removeRoads();map.remove();};
      ...
      projects.forEach(p=>{const plan=parseMap(p);if(!plan)return;const position=plan.features[0]?.points[0]||plan.center;points.push(position);
        const button=document.createElement("span");button.className="map-project-pin";button.textContent=p.serviceType==="consultation"?"◎":p.serviceType==="native-landscape"?"🌿":"🌾";button.setAttribute("aria-hidden","true");
        const client=clients.find(c=>c.id===p.clientId);
        const label=document.createElement("span");label.textContent=p.name+(client?" · "+client.firstName+" "+client.lastName:"");
