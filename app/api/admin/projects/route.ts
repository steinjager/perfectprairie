...
import { choice, date, id, projectStatuses, revision, services, text } from "@/lib/operations";
...
async function save(request: Request, editing: boolean) {
  try {
    ...
    const values = {
      ...
      publicSummary: text(input.publicSummary, "Customer summary", 4000), targetStartDate: date(input.targetStartDate, "Target start"),
    };
    if (!editing) {
