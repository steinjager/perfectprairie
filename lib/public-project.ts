...
export async function customerProject(token: string) {
  ...
  const fields={id:estimates.id,number:estimates.number,status:estimates.status,issueDate:estimates.issueDate,dueDate:estimates.validUntil,notes:estimates.notes,totalCents:estimates.totalCents};
  ...
  return {name:project.name,summary:project.publicSummary,map:parseMap(project),documents};
}
